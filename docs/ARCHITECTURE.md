...
### 1.3 UAT 同機部署（Phase 2 — 現況）

App + Supabase 皆跑 UAT VM，`https://uat.gf-v.io` 為單一入口：

```
nginx (proxy) ──► mada-next:3000        (Next.js)
              └──► supabase-kong:8000   (Auth / REST / Storage / Realtime)
```

Deploy：`npm run deploy:uat:next`（詳見 [`DEPLOY_SELFHOST.md`](./DEPLOY_SELFHOST.md)）。

Production 仍可使用 Vercel + Supabase Cloud；UAT 為自建完整堆疊。
...
- **函式庫**：[`next-intl`](https://next-intl.dev) 4.x；plugin 入口 `src/i18n/request.ts`
...
| 22 | Listings 沒有圖片上傳 UI（`listings.images jsonb` 早就在 schema，但 ListingForm 從不讓賣家放東西進去） | ✅ 已完成（2026-05-24） | **migration 024** 建立 `listings` public storage bucket（2 MiB / JPEG/PNG/WebP 白名單）+ 4 條 RLS（公開讀；owner 路徑寫/改；owner+admin 刪）。新元件 `<ListingImageUploader />`：drag-drop + 「From your library」可重用既往上傳（`listMyListingImages` server action 列出 `listings/{auth.uid()}/...`）。**Client-side `compressTo720pWebp`**（`src/lib/images/compress.ts`）在上傳前把長邊縮到 720 px、重編碼成 WebP（q=0.82），實測 12 KB PNG → 1.2 KB（~90% reduction）。`<MarketListingCard />` 有圖時加 16:9 banner、`/market/[id]` 用新 `<ListingGallery />` hero + thumbnail。10 個 verify-schema bucket assertion + 9 個 smoke-listing-images RLS assertion 全綠 |
| 23 | 賣家上完貨之後沒有任何 UI 可以編輯或刪除（`updateListing` 只接 partial Status，沒有 `deleteListing`，`/listings` 列表沒按鈕） | ✅ 已完成（2026-05-24） | `updateListing(id, input)` 改 dual-mode（full-form edit 跑完整 zod + commercial / KYC gate；bare status-toggle 沿用原 path），加 ownership pre-check 回 `NOT_FOUND_OR_FORBIDDEN`。新 `deleteListing(id)` server action：order-attached listing 回 `LISTING_HAS_ORDERS`、其餘 hard delete（`inquiries` / `quotations` FK 自然 set null）。新頁 `/listings/[id]/edit` 復用 `<ListingForm existing={...}>`；`/listings` 加 Actions 欄 `<ListingRowActions />`（Edit / Pause↔Resume / Sold out / Delete + confirm dialog）。11 個 smoke-listing-delete assertion 全綠 |
