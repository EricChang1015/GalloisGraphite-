...
### A1. ⚠️ Schema 對齊（最高優先 — 不修會導致 production runtime error）

代碼／TS types 跟 `001_init.sql` 對不上，必須以新 migration 修正：

- [ ] 新增 [`supabase/migrations/005_align_payments_and_news.sql`](../supabase/migrations/005_align_payments_and_news.sql)
  - `payments.payer_id` → `buyer_id`（rename，並更新 RLS policy）
  - `payments` 加 `admin_note text`、`reviewed_by uuid → profiles(id)`、`reviewed_at timestamptz`
  - 既存 `verified_by` / `verified_at` 改為 generated column 或同步
  - `news` 加 `author_id uuid → profiles(id)`
  - `orders` 加 `updated_at timestamptz` + `BEFORE UPDATE` trigger
- [ ] `docs/SCHEMA.md` 同步更新（payments、news、orders 欄位）
- [ ] 執行後重跑 `npx supabase gen types typescript ... > src/types/database.ts`
...
- [ ] A1 schema 對齊全部完成、TS types 重新生成
- [ ] A2 IM 可雙方即時對話 + 圖片附件
...
- [ ] 公開頁 SEO meta（title/description/og）齊全 ✅（已完成）
- [ ] 所有路由都有 `loading.tsx` 與 `error.tsx` 雛形 ✅（多數已完成）
